...
class FileController(config: ConfigProperties) {
    ...
    fun getFiles(
        model: Model,
        @PathVariable("path", required = false) inputPath: String?
    ): Mono<ResponseWrap<MutableList<FileInfo>>> {
        val path: String = inputPath.orEmpty()
    ...
    fun download(@PathVariable("path", required = false) inputPath: String?, response: ServerHttpResponse): Mono<Void> {
        val path: String = inputPath.orEmpty()
        try {
